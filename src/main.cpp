...
extern "C" size_t get_value_size(byte_t*, size_t);
extern "C" byte_t* get_storage(byte_t*, size_t, byte_t*);
extern "C" void set_storage(byte_t*, size_t, byte_t*, size_t);

extern "C" void print_bytes(byte_t*, size_t);

byte_t* get_data(byte_t* key, size_t key_size) {
  int size = get_value_size(key, key_size);
  byte_t* ret = new byte_t[size];
  get_storage(key, key_size, ret);
  return ret;
}
...
void emit(int price, int quantity, OrderSide side){
    int length = snprintf(NULL, 0, "match %d %d %s", price, quantity, side == BUY_SIDE ? "buy" : "sell");
    char log[length];
    sprintf(log, "match %d %d %s", price, quantity, side == BUY_SIDE ? "buy" : "sell");
    print_bytes((uint8_t*)log, sizeof(log));
}
...
OrderBook get_book(){
    int book_size = int_from_bytes(get_data((uint8_t*)"book_size", 9));
    byte_t* serialized_book = get_data((uint8_t*)"order_book", 10);
...
void set_book(OrderBook book){
    ...
    set_storage((uint8_t*)"book_size", 9, (uint8_t*)&book_size, 4);
    set_storage((uint8_t*)"order_book", 10, bytes, book_size * sizeof(Order));
}
