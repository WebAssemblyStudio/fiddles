...
public: 
    void add(Order order){
        if(order.side == SELL_SIDE){
            ...
            if(order.quantity > 0){
                char log[32];
                sprintf(log, "precheck %d %d %d %d", sell_queue.size(), best_ask(), buy_queue.size(), best_bid());
                print_bytes(0, strbytes(log));      
        ...
        print_bytes(0, strbytes(log));
        sprintf(log, "chkaddr %d %d", &sell_queue, &buy_queue);
        print_bytes(0, strbytes(log));
        if(sell_queue.size() > 0 && buy_queue.size()>0){
          sprintf(log, "chktop %d %d", &(sell_queue.top()), &(buy_queue.top()));
          print_bytes(0, strbytes(log));
        }
