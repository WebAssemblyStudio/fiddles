(module
  ...
  (global $width i32 i32.const 200)
  (global $height i32 i32.const 200)
...
(func $xorshift32 (result i32)
  ...
  (func $loopcol (param $y i32) (param $mf i32)
    (local $x i32)
    ...
    (block 
      (loop
        ...
        (br_if 1 (i32.eq (get_local $x) (get_global $width)))
        (br 0)
      )
    )
  )

  (func $looprow (param $mf i32)
    (local $y i32)
    ...
    (block 
      (loop
        ...
        (call $loopcol (get_local $y) (get_local $mf))
        (set_local $y (call $increment (get_local $y)))
      
        (br_if 1 (i32.eq (get_local $y) (get_global $height)))
        (br 0)
      )
    )
  )
  ...
  (func $main (param $mapfunc i32) (result i32)
    ...
    (call $looprow (get_local $mapfunc))
