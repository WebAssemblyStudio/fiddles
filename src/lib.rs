// Current prelude for using `wasm_bindgen`, and this'll get smaller over time!
#![feature(proc_macro, wasm_custom_section, wasm_import_module, const_fn)]
extern crate wasm_bindgen;

/*
 * A speed-improved simplex noise algorithm for 2D, 3D and 4D in Java.
 *
 * Based on example code by Stefan Gustavson (stegu@itn.liu.se).
 * Optimisations by Peter Eastman (peastman@drizzle.stanford.edu).
 * Better rank ordering method for 4D by Stefan Gustavson in 2012.
 *
 * This could be speeded up even further, but it's useful as it is.
 *
 * Version 2012-03-09
 *
 * This code was placed in the public domain by its original author,
 * Stefan Gustavson. You may use it as you see fit, but
 * attribution is appreciated.
 *
 */

// Simplex noise in 2D, 3D and 4D
pub mod SimplexNoise {
  use wasm_bindgen::prelude::*;

	fn tesseract(
		x: f64
	) -> f64 {
		x * x * x * x
	}

	// Inner class to speed up gradient computations
	// (In Java, array access is a lot slower than member access)
	struct Grad {
		x: f64,
		y: f64,
		z: f64,
		w: f64
	}

	impl Grad {
		const fn new(
			x: i8,
			y: i8,
			z: i8,
			w: i8
		) -> Self {
			Self {
				x: x as f64,
				y: y as f64,
				z: z as f64,
				w: w as f64
			}
		}
	}

	const grad3: [Grad; 12] = [
		Grad::new( 1,  1,  0,  0),
		Grad::new(-1,  1,  0,  0),
		Grad::new( 1, -1,  0,  0),
		Grad::new(-1, -1,  0,  0),
		Grad::new( 1,  0,  1,  0),
		Grad::new(-1,  0,  1,  0),
		Grad::new( 1,  0, -1,  0),
		Grad::new(-1,  0, -1,  0),
		Grad::new( 0,  1,  1,  0),
		Grad::new( 0, -1,  1,  0),
		Grad::new( 0,  1, -1,  0),
		Grad::new( 0, -1, -1,  0)
	];

	const grad4: [Grad; 32] = [
		Grad::new( 0,  1,  1,  1),
		Grad::new( 0,  1,  1, -1),
		Grad::new( 0,  1, -1,  1),
		Grad::new( 0,  1, -1, -1),
		Grad::new( 0, -1,  1,  1),
		Grad::new( 0, -1,  1, -1),
		Grad::new( 0, -1, -1,  1),
		Grad::new( 0, -1, -1, -1),
		Grad::new( 1,  0,  1,  1),
		Grad::new( 1,  0,  1, -1),
		Grad::new( 1,  0, -1,  1),
		Grad::new( 1,  0, -1, -1),
		Grad::new(-1,  0,  1,  1),
		Grad::new(-1,  0,  1, -1),
		Grad::new(-1,  0, -1,  1),
		Grad::new(-1,  0, -1, -1),
		Grad::new( 1,  1,  0,  1),
		Grad::new( 1,  1,  0, -1),
		Grad::new( 1, -1,  0,  1),
		Grad::new( 1, -1,  0, -1),
		Grad::new(-1,  1,  0,  1),
		Grad::new(-1,  1,  0, -1),
		Grad::new(-1, -1,  0,  1),
		Grad::new(-1, -1,  0, -1),
		Grad::new( 1,  1,  1,  0),
		Grad::new( 1,  1, -1,  0),
		Grad::new( 1, -1,  1,  0),
		Grad::new( 1, -1, -1,  0),
		Grad::new(-1,  1,  1,  0),
		Grad::new(-1,  1, -1,  0),
		Grad::new(-1, -1,  1,  0),
		Grad::new(-1, -1, -1,  0)
	];

	// temporary lookup table
	const p: [u8; 0x100] = [
		
	];

	// To remove the need for index wrapping, double the permutation table length
	static mut perm: [u8; 0x200] = [0; 0x200];
	static mut permMod12: [u8; 0x200] = [0; 0x200];

	// Skewing and unskewing factors for 2, 3, and 4 dimensions
	static mut F2: f64 = 0.0;
	static mut G2: f64 = 0.0;
	static mut F3: f64 = 0.0;
	static mut G3: f64 = 0.0;
	static mut F4: f64 = 0.0;
	static mut G4: f64 = 0.0;

	pub unsafe fn init() {
		F2 = (f64::sqrt(3.0) - 1.0) / 2.0;
		G2 = (3.0 - f64::sqrt(3.0)) / 6.0;
		F3 = 1.0 / 3.0;
		G3 = 1.0 / 6.0;
		F4 = (f64::sqrt(5.0) - 1.0) / 4.0;
		G4 = (5.0 - f64::sqrt(5.0)) / 20.0;

		// copy p[] into first and second half of perm
		for i in 0x0_usize..0x200 {
			perm[i] = p[i & 0xFF]; // wrap back to start of `p`
			permMod12[i] = perm[i] % 12;
		}
	}

	// This method is a *lot* faster than using (int)Math.floor(x)
	fn fastfloor(
		x: f64
	) -> i32 {
		let xi: i32 = x as i32;

		xi - (x < xi as f64) as i32
	}

	// dot_2D
	fn dot_3(
		g: &Grad,
		x: f64,
		y: f64
	) -> f64 {
		g.x * x + g.y * y
	}

	// dot_3D
	fn dot_4(
		g: &Grad,
		x: f64,
		y: f64,
		z: f64
	) -> f64 {
		g.x * x + g.y * y + g.z * z
	}

	// dot_4D
	fn dot_5(
		g: &Grad,
		x: f64,
		y: f64,
		z: f64,
		w: f64
	) -> f64 {
		g.x * x + g.y * y + g.z * z + g.w * w
	}

	// noise_2D
	// 2D simplex noise
  #[wasm_bindgen]
	pub fn noise_2(
		xin: f64,
		yin: f64
	) -> f64 {
    unsafe {
		// Skew the input space to determine which simplex cell we're in
		let i: i32;
		let j: i32;
		{
			let s: f64 = (xin + yin) * F2; // Hairy factor for 2D
			i = fastfloor(xin + s);
			j = fastfloor(yin + s);
		}
		let X0: f64;
		let Y0: f64;
		{
			let t: f64 = (i + j) as f64 * G2;
			X0 = i as f64 - t; // Unskew the cell origin back to (x, y) space
			Y0 = j as f64 - t;
		}
		let x0: f64 = xin - X0; // The x, y distances from the cell origin
		let y0: f64 = yin - Y0;
		// For the 2D case, the simplex shape is an equilateral triangle.
		// Determine which simplex we are in.

		// A step of (1, 0) in (i, j) means a step of (1 - c, -c) in (x, y), and
		// a step of (0, 1) in (i, j) means a step of (-c, 1 - c) in (x, y), where
		// c = (3 - sqrt(3)) / 6

		// Work out the hashed gradient indices of the three simplex corners
		let ii: u8 = (i & 0xFF) as u8;
		let jj: u8 = (j & 0xFF) as u8;
		// Calculate the contribution from the three corners

		// Noise contributions from the three corners, n0..n2

		let n0: f64;
		{
			let t0: f64 = 0.5 - x0 * x0 - y0 * y0;
			if t0 < 0.0 {
				n0 = 0.0;
			} else {
				let gi0: u8 = permMod12[ii as usize + perm[jj as usize] as usize];

				n0 = tesseract(t0) * dot_3(&grad3[gi0 as usize], x0, y0); // (x, y) of grad3 used for 2D gradient
			}
		}

		let n1: f64;
		{
			let i1: u8;
			let j1: u8; // Offsets for second (middle) corner of simplex in (i, j) coords
			if x0 > y0 {
				// lower triangle, XY order: (0, 0)->(1, 0)->(1, 1)
				i1 = 1;
				j1 = 0;
			} else {
				// upper triangle, YX order: (0, 0)->(0, 1)->(1, 1)
				i1 = 0;
				j1 = 1;
			}

			let x1: f64 = x0 - i1 as f64 + G2; // Offsets for middle corner in (x, y) unskewed coords
			let y1: f64 = y0 - j1 as f64 + G2;

			let t1: f64 = 0.5 - x1 * x1 - y1 * y1;
			if t1 < 0.0 {
				n1 = 0.0;
			} else {
				let gi1: u8 = permMod12[ii as usize + i1 as usize + perm[jj as usize + j1 as usize] as usize];

				n1 = tesseract(t1) * dot_3(&grad3[gi1 as usize], x1, y1);
			}
		}

		let n2: f64;
		{
			let x2: f64 = x0 - 1.0 + 2.0 * G2; // Offsets for last corner in (x, y) unskewed coords
			let y2: f64 = y0 - 1.0 + 2.0 * G2;
			let t2: f64 = 0.5 - x2 * x2 - y2 * y2;

			if t2 < 0.0 {
				n2 = 0.0;
			} else {
				let gi2: u8 = permMod12[ii as usize + 1 + perm[jj as usize + 1] as usize];

				n2 = tesseract(t2) * dot_3(&grad3[gi2 as usize], x2, y2);
			}
		}

		// Add contributions from each corner to get the final noise value.
		// The result is scaled to return values in the i32erval [-1, 1].
		70.0 * (n0 + n1 + n2)
    }
	}

	// noise_3D
	// 3D simplex noise
  #[wasm_bindgen]
	pub fn noise_3(
		xin: f64,
		yin: f64,
		zin: f64
	) -> f64 {
    unsafe {
		// Skew the input space to determine which simplex cell we're in
		let i: i32;
		let j: i32;
		let k: i32;
		{
			let s: f64 = (xin + yin + zin) * F3; // Very nice and simple skew factor for 3D
			i = fastfloor(xin + s);
			j = fastfloor(yin + s);
			k = fastfloor(zin + s);
		}
		let X0: f64;
		let Y0: f64;
		let Z0: f64;
		{
			let t: f64 = (i + j + k) as f64 * G3;
			X0 = i as f64 - t; // Unskew the cell origin back to (x, y, z) space
			Y0 = j as f64 - t;
			Z0 = k as f64 - t;
		}
		let x0: f64 = xin - X0; // The x, y, z distances from the cell origin
		let y0: f64 = yin - Y0;
		let z0: f64 = zin - Z0;
		// For the 3D case, the simplex shape is a slightly irregular tetrahedron.
		// Determine which simplex we are in.

		// Offsets for second corner of simplex in (i, j, k) coords
		let i1: u8;
		let j1: u8;
		let k1: u8;
		// Offsets for third corner of simplex in (i, j, k) coords
		let i2: u8;
		let j2: u8;
		let k2: u8;

		{
			let arr: (u8, u8, u8, u8, u8, u8);

			if x0 >= y0 {
				if y0 >= z0 {
					// X Y Z order
					arr = ( 1, 0, 0, 1, 1, 0 );
				} else if x0 >= z0 {
					// X Z Y order
					arr = ( 1, 0, 0, 1, 0, 1 );
				} else {
					// Z X Y order
					arr = ( 0, 0, 1, 1, 0, 1 );
				}
			} else {
				// x0 < y0
				if y0 < z0 {
					// Z Y X order
					arr = ( 0, 0, 1, 0, 1, 1 );
				} else if x0 < z0 {
					// Y Z X order
					arr = ( 0, 1, 0, 0, 1, 1 );
				} else {
					// Y X Z order
					arr = ( 0, 1, 0, 1, 1, 0 );
				}
			}

			i1 = arr.0;
			j1 = arr.1;
			k1 = arr.2;
			i2 = arr.3;
			j2 = arr.4;
			k2 = arr.5;
		}

		// A step of (1, 0, 0) in (i, j, k) means a step of (1 - c,    -c,    -c) in (x, y, z),
		// a step of (0, 1, 0) in (i, j, k) means a step of (   -c, 1 - c,    -c) in (x, y, z), and
		// a step of (0, 0, 1) in (i, j, k) means a step of (   -c,    -c, 1 - c) in (x, y, z), where
		// c = 1 / 6.

		// Work out the hashed gradient indices of the four simplex corners
		let ii: u8 = (i & 0xFF) as u8;
		let jj: u8 = (j & 0xFF) as u8;
		let kk: u8 = (k & 0xFF) as u8;

		// Calculate the contribution from the four corners

		let n0: f64;
		{
			let t0: f64 = 0.6 - x0 * x0 - y0 * y0 - z0 * z0;

			if t0 < 0.0 {
				n0 = 0.0;
			} else {
				let gi0: u8 = permMod12[ii as usize + perm[jj as usize + perm[kk as usize] as usize] as usize];

				n0 = tesseract(t0) * dot_4(&grad3[gi0 as usize], x0, y0, z0);
			}
		}

		let n1: f64;
		{
			let x1: f64 = x0 - i1 as f64 + G3; // Offsets for second corner in (x, y, z) coords
			let y1: f64 = y0 - j1 as f64 + G3;
			let z1: f64 = z0 - k1 as f64 + G3;

			let t1: f64 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1;

			if t1 < 0.0 {
				n1 = 0.0;
			} else {
				let gi1: u8 = permMod12[ii as usize + i1 as usize + perm[jj as usize + j1 as usize + perm[kk as usize + k1 as usize] as usize] as usize];

				n1 = tesseract(t1) * dot_4(&grad3[gi1 as usize], x1, y1, z1);
			}
		}

		let n2: f64;
		{
			let x2: f64 = x0 - i2 as f64 + 2.0 * G3; // Offsets for third corner in (x, y, z) coords
			let y2: f64 = y0 - j2 as f64 + 2.0 * G3;
			let z2: f64 = z0 - k2 as f64 + 2.0 * G3;

			let t2: f64 = 0.6 - x2 * x2 - y2 * y2 - z2 * z2;

			if t2 < 0.0 {
				n2 = 0.0;
			} else {
				let gi2: u8 = permMod12[ii as usize + i2 as usize + perm[jj as usize + j2 as usize + perm[kk as usize + k2 as usize] as usize] as usize];

				n2 = tesseract(t2) * dot_4(&grad3[gi2 as usize], x2, y2, z2);
			}
		}

		let n3: f64;
		{
			let x3: f64 = x0 - 1.0 + 3.0 * G3; // Offsets for last corner in (x, y, z) coords
			let y3: f64 = y0 - 1.0 + 3.0 * G3;
			let z3: f64 = z0 - 1.0 + 3.0 * G3;

			let t3: f64 = 0.6 - x3 * x3 - y3 * y3 - z3 * z3;

			if t3 < 0.0 {
				n3 = 0.0;
			} else {
				let gi3: u8 = permMod12[ii as usize + 1 + perm[jj as usize + 1 + perm[kk as usize + 1] as usize] as usize];

				n3 = tesseract(t3) * dot_4(&grad3[gi3 as usize], x3, y3, z3);
			}
		}

		// Add contributions from each corner to get the final noise value.
		// The result is scaled to stay just inside [-1, 1]
		32.0 * (n0 + n1 + n2 + n3)
    }
	}

	// noise_4D
	// 4D simplex noise, better simplex rank ordering method 2012-03-09
  #[wasm_bindgen]
	pub fn noise_4(
		x: f64,
		y: f64,
		z: f64,
		w: f64
	) -> f64 {
    unsafe {
		// Skew the (x, y, z, w) space to determine which cell of 24 simplices we're in
		let i: i32;
		let j: i32;
		let k: i32;
		let l: i32;
		{
			let s: f64 = (x + y + z + w) * F4; // Factor for 4D skewing
			i = fastfloor(x + s);
			j = fastfloor(y + s);
			k = fastfloor(z + s);
			l = fastfloor(w + s);
		}
		let W0: f64;
		let Z0: f64;
		let Y0: f64;
		let X0: f64;
		{
			let t: f64 = (i + j + k + l) as f64 * G4; // Factor for 4D unskewing
			X0 = i as f64 - t; // Unskew the cell origin back to (x, y, z, w) space
			Y0 = j as f64 - t;
			Z0 = k as f64 - t;
			W0 = l as f64 - t;
		}
		let x0: f64 = x - X0; // The x, y, z, w distances from the cell origin
		let y0: f64 = y - Y0;
		let z0: f64 = z - Z0;
		let w0: f64 = w - W0;

		let i1: u8;
		let j1: u8;
		let k1: u8;
		let l1: u8;
		let i2: u8;
		let j2: u8;
		let k2: u8;
		let l2: u8;
		let i3: u8;
		let j3: u8;
		let k3: u8;
		let l3: u8;
		{
			// For the 4D case, the simplex is a 4D shape I won't even try to describe.
			// To find out which of the 24 possible simplices we're in, we need to
			// determine the magnitude ordering of x0, y0, z0 and w0.
			// Six pair-wise comparisons are performed between each possible pair
			// of the four coordinates, and the results are used to rank the numbers.
			let mut rankx: u8 = 0;
			let mut ranky: u8 = 0;
			let mut rankz: u8 = 0;
			let mut rankw: u8 = 0;
			if x0 > y0 {
				rankx += 1;
			} else {
				ranky += 1;
			}

			if x0 > z0 {
				rankx += 1;
			} else {
				rankz += 1;
			}

			if x0 > w0 {
				rankx += 1;
			} else {
				rankw += 1;
			}

			if y0 > z0 {
				ranky += 1;
			} else {
				rankz += 1;
			}

			if y0 > w0 {
				ranky += 1;
			} else {
				rankw += 1;
			}

			if z0 > w0 {
				rankz += 1;
			} else {
				rankw += 1;
			}

			// [rankx, ranky, rankz, rankw] is a 4-vector with the numbers 0, 1, 2 and 3
			// in some order. We use a thresholding to set the coordinates in turn.
			// The i32eger offsets for the second simplex corner
			// Rank 3 denotes the largest coordinate.
			i1 = (rankx >= 3) as u8;
			j1 = (ranky >= 3) as u8;
			k1 = (rankz >= 3) as u8;
			l1 = (rankw >= 3) as u8;
			// The i32eger offsets for the third simplex corner
			// Rank 2 denotes the second largest coordinate.
			i2 = (rankx >= 2) as u8;
			j2 = (ranky >= 2) as u8;
			k2 = (rankz >= 2) as u8;
			l2 = (rankw >= 2) as u8;
			// The i32eger offsets for the fourth simplex corner
			// Rank 1 denotes the second smallest coordinate.
			i3 = (rankx >= 1) as u8;
			j3 = (ranky >= 1) as u8;
			k3 = (rankz >= 1) as u8;
			l3 = (rankw >= 1) as u8;
		}
		// The fifth corner has all coordinate offsets = 1, so no need to compute that.

		// Work out the hashed gradient indices of the five simplex corners
		let ii: u8 = (i & 0xFF) as u8;
		let jj: u8 = (j & 0xFF) as u8;
		let kk: u8 = (k & 0xFF) as u8;
		let ll: u8 = (l & 0xFF) as u8;

		// Calculate the contribution from the five corners
		let n0: f64;
		{
			let t0: f64 = 0.6 - x0 * x0 - y0 * y0 - z0 * z0 - w0 * w0;

			if t0 < 0.0 {
				n0 = 0.0;
			} else {
				let gi0: u8 = perm[ii as usize + perm[jj as usize + perm[kk as usize + perm[ll as usize] as usize] as usize] as usize] % 32;

				n0 = tesseract(t0) * dot_5(&grad4[gi0 as usize], x0, y0, z0, w0);
			}
		}

		let n1: f64;
		{
			let x1: f64 = x0 - i1 as f64 + G4; // Offsets for second corner in (x, y, z, w) coords
			let y1: f64 = y0 - j1 as f64 + G4;
			let z1: f64 = z0 - k1 as f64 + G4;
			let w1: f64 = w0 - l1 as f64 + G4;

			let t1: f64 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1 - w1 * w1;

			if t1 < 0.0 {
				n1 = 0.0;
			} else {
				let gi1: u8 = perm[ii as usize + i1 as usize + perm[jj as usize + j1 as usize + perm[kk as usize + k1 as usize + perm[ll as usize + l1 as usize] as usize] as usize] as usize] % 32;

				n1 = tesseract(t1) * dot_5(&grad4[gi1 as usize], x1, y1, z1, w1);
			}
		}

		let n2: f64;
		{
			let x2: f64 = x0 - i2 as f64 + 2.0 * G4; // Offsets for third corner in (x, y, z, w) coords
			let y2: f64 = y0 - j2 as f64 + 2.0 * G4;
			let z2: f64 = z0 - k2 as f64 + 2.0 * G4;
			let w2: f64 = w0 - l2 as f64 + 2.0 * G4;

			let t2: f64 = 0.6 - x2 * x2 - y2 * y2 - z2 * z2 - w2 * w2;

			if t2 < 0.0 {
				n2 = 0.0;
			} else {
				let gi2: u8 = perm[ii as usize + i2 as usize + perm[jj as usize + j2 as usize + perm[kk as usize + k2 as usize + perm[ll as usize + l2 as usize] as usize] as usize] as usize] % 32;

				n2 = tesseract(t2) * dot_5(&grad4[gi2 as usize], x2, y2, z2, w2);
			}
		}

		let n3: f64;
		{
			let x3: f64 = x0 - i3 as f64 + 3.0 * G4; // Offsets for fourth corner in (x, y, z, w) coords
			let y3: f64 = y0 - j3 as f64 + 3.0 * G4;
			let z3: f64 = z0 - k3 as f64 + 3.0 * G4;
			let w3: f64 = w0 - l3 as f64 + 3.0 * G4;

			let t3: f64 = 0.6 - x3 * x3 - y3 * y3 - z3 * z3 - w3 * w3;

			if t3 < 0.0 {
				n3 = 0.0;
			} else {
				let gi3: u8 = perm[ii as usize + i3 as usize + perm[jj as usize + j3 as usize + perm[kk as usize + k3 as usize + perm[ll as usize + l3 as usize] as usize] as usize] as usize] % 32;

				n3 = tesseract(t3) * dot_5(&grad4[gi3 as usize], x3, y3, z3, w3);
			}
		}

		let n4: f64;
		{
			let x4: f64 = x0 - 1.0 + 4.0 * G4; // Offsets for last corner in (x, y, z, w) coords
			let y4: f64 = y0 - 1.0 + 4.0 * G4;
			let z4: f64 = z0 - 1.0 + 4.0 * G4;
			let w4: f64 = w0 - 1.0 + 4.0 * G4;

			let t4: f64 = 0.6 - x4 * x4 - y4 * y4 - z4 * z4 - w4 * w4;

			if t4 < 0.0 {
				n4 = 0.0;
			} else {
				let gi4: u8 = perm[ii as usize + 1 + perm[jj as usize + 1 + perm[kk as usize + 1 + perm[ll as usize + 1] as usize] as usize] as usize] % 32;

				n4 = tesseract(t4) * dot_5(&grad4[gi4 as usize], x4, y4, z4, w4);
			}
		}

		// Sum up and scale the result to cover the range [-1, 1]
		27.0 * (n0 + n1 + n2 + n3 + n4)
    }
	}
}

fn main() {
	unsafe {
		SimplexNoise::init();
	};
}
