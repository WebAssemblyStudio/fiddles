...
pub fn greet(name: &str) {
    use wasm_bindgen::__rt::core::mem::size_of_val;
    ...
    let mut fishs: Vec<u8> = input.split(",").map(|n| n.parse().unwrap()).collect();
    
    const DAYS: u32 = 256;
    
    for current_day in 0..DAYS {
        let mut fishs_to_add = 0;
        ...
        let fishs_to_add = vec![8u8; fishs_to_add];
        fishs.extend(&fishs_to_add);

        log(&format!("Size of fishs: {}", size_of_val(&*fishs)));
        log(&format!("After {} day(s): -> {}", current_day+1/*, fishs*/, fishs.len()));
    }
}
