    ...
    for (index, frame) in animation.chunks_mut(rows * cols).enumerate() {
        for row in 0..rows {
            for col in 0..cols {
                ...
                let x = col as f64 - idx / 10.0 - 18.0;
                let y = row as f64 + idx / 8.0 - 22.0;
                let wobble = sine(idx, 100.0);
                let mut r = sine((x * x + y * y + idx / 2.0), 80.0) * 60.0 + 100.0;
                let mut g = sine((x * x + y * y + idx / (2.0 + wobble)), 80.0) * 60.0 + 100.0;
                let mut b = sine((x * x + y * y + idx / (2.0 + wobble / 2.0)), 80.0) * 60.0 + 100.0;
