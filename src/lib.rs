    ...
    for (index, frame) in animation.chunks_mut(rows * cols).enumerate() {
        for row in 0..rows {
            for col in 0..cols {
                ...
                let x = row as f64;
                let y = sine(col as f64 + idx / 10.0, 20.0) * 10.0;
                let mut r = sine(x + y + idx/2.0, 90.0) * 100.0 + 128.0;
                let mut g = sine(x + y + idx/2.0 + 30.0, 90.0) * 100.0 + 128.0;
                let mut b = sine(x + y + idx/2.0 + 60.0, 90.0) * 100.0 + 128.0;
