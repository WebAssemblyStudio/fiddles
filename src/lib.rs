...
pub fn prepareCropByRust (  
    ...
    crop_mode:          u8) -> String {
        ...
        let main_focusx      = (main_focus_centerx - (main_focus_width / 2.0)) - crop_left ;
        let main_focus_right = main_focusx + main_focus_width;

        let main_focusy       = (main_focus_centery - (main_focus_height / 2.0)) - crop_top ;
	    let main_focus_bottom = main_focusy + main_focus_height;
		...
		let target_aspect_ratio  : f64 = ratio_width / ratio_height;
		...
		if aspect_ratio_kind == "s" {
			my_resize  = format!("{}:{} ", ratio_width.round(), ratio_height.round()); 
		}
