    ...
    for y in (posy..(posy+8)) {
        for x in (posx..(posx+8)).rev() {
            ...
            if bit == 1 {
                ...
                buf[pos(xu, y)] = Rgb::new(251, 21, 83);
            }
        }
    }
}
...
pub extern fn apply() {
    ...
    clear(animation, Rgb::new(251, 21, 83));
    let mut offset = 0;
    ...
    for (index, frame) in animation.chunks_mut(rows * cols).enumerate() {
      add_string_trimmed(cols as isize - offset, 0, index, frame);
      // if index < 200 {
        // add_string_trimmed(cols as isize - offset, 0, str1, frame);
      // } 
      // else if (frame > 20 && frame < 30) {
      //   add_string_trimmed(cols as isize - offset, 0, str2, frame);
      // } else if (frame > 30 && frame < 40) {
      //   add_string_trimmed(cols as isize - offset, 0, str3, frame);
      // } else if (frame > 40) {
      //   add_string_trimmed(cols as isize - offset, 0, "REST", frame);
      // }
    }

    // for (index, frame) in animation.chunks_mut(rows * cols).enumerate() {
    //     add_string_trimmed(cols as isize - offset, 0, "COPPER | The crm that works for you", frame);
    //     // add_string_trimmed(cols as isize - offset, 10, "LOVES", frame);
    //     // add_string_trimmed(cols as isize - offset, 20, "TUCE", frame);
    //     // add_string_flipped(-(8*8) + offset, rows-25, "The crm that works for you", frame);
    //     if index%2 == 0 {
    //       offset += 1;
    //     }
    // }
