    ...
    for (index, frame) in animation.chunks_mut(rows * cols).enumerate() {
        for row in 0..rows {
            for col in 0..cols {
                ...
                let x = sine(col as f64 - 16.0, 10.0);
                let y = sine(row as f64 - 22.0, 10.0);
                let mut r = sine(x + y + idx / 100.0, 80.0) * 100.0 + 128.0;
                let mut g = sine(x + y + idx / 100.0, 80.0) * 100.0 + 128.0;
                let mut b = sine(x + y + idx / 100.0, 80.0) * 100.0 + 128.0;
