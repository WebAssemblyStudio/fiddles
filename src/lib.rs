    ...
    for (index, frame) in animation.chunks_mut(rows * cols).enumerate() {
        for row in 0..rows {
            for col in 0..cols {
                ...
                let y = row as f64 - 22.0 - idx / 5.0;
                let mut r = sine((x * x + y * y + idx / 20.0), 80.0) * 100.0 + 128.0;
                let mut g = sine((x * x + y * y + idx / 20.0 - 1.0), 80.0) * 100.0 + 128.0;
                let mut b = sine((x * x + y * y + idx / 20.0 - 2.0), 80.0) * 100.0 + 128.0;
