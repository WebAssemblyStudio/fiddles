    ...
    for (index, frame) in animation.chunks_mut(rows * cols).enumerate() {
        for y in 0..rows {
          for x in 0..cols {
            ...
            let mut z = (x as f32/(cols*2) as f32, y as f32/(rows*2) as f32);
            let max_its = 50;
            let mut its = max_its;
            for _ in 0..max_its {
              its -= 1;
              z = complex_mul(z,z);
              z.0 += (x as f32/(cols*2) as f32, y as f32/(rows*2) as f32).0;
              z.1 += (x as f32/(cols*2) as f32, y as f32/(rows*2) as f32).1;
