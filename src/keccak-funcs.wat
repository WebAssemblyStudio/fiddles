...
;; ****
;; this is a copy of keccak.wat, but without the opening paren
;; module declaration, and without the closing paren.
;; ****
...
(func $keccak_update
  ...
  (if (i32.ne (get_local $residue_index) (i32.const 0))
    (then
      ...
      (drop) ;; cdetrio: memcpy returns a value, not sure why
  ...
  (if (i32.gt_u (get_local $input_length) (i32.const 0))
    (then
      ...
      (drop) ;; cdetrio: memcpy returns a val
...
(func $keccak_finish
  ...
  (drop) ;; cdetrio: memset also returns a val
...
;;
(func $keccak/keccak ;; this name needs to match what assemblyscript generates for the import
  (param $context_offset i32)
  (param $input_offset i32)
  (param $input_length i32)
  (param $output_offset i32)

  (call $main/sayHello (i32.const 69))
