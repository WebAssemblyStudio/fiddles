...
(function(){
	...
	$proto.run = function() {
		...
		this.cpuTimeout = window.setTimeout(
			this.$run, Math.max(this.machine.waitMillis - Date.now(), 10), this
		);
	};

	$proto.wait = function(x) {
		if (this.machine.waitMillis === -1) {
			this.machine.waitMillis = 0;
		}
		else {
			this.machine.waitMillis = this.startMillis + x;
		}
	};
