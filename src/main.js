fetch('../out/main.wasm').then(response =>
  ...
  for (var i = 0; i < ns; ++i) {
    ...
    floats[o + 1] = 0;
    o += ns * 2;
    floats[o] = Math.sin(i / ns * Math.PI * 2);
    floats[o + 1] = 0;
  }
  ...
  const pns = Math.min(ns, 16);
  console.log("1o: "+cmp(floats.slice(0, pns * 2)));
  console.log("2o: "+cmp(floats.slice(ns * 2, (ns + pns) * 2)));
  let start = window.performance.now();
  instance.exports.multidfft(floats.byteOffset, ns, -1, 2);
  let end = window.performance.now();
  console.log("1f: "+cmp(floats.slice(0, pns * 2)));
  console.log("2f: "+cmp(floats.slice(ns * 2, (ns + pns) * 2)));
  instance.exports.multidfft(floats.byteOffset, ns, 1, 2);
  const invns = 1/ns;
  for (var i = 0; i < 4*ns; ++i) {
    floats[i] *= invns;
  }
  console.log("1i: "+cmp(floats.slice(0, pns * 2)));
  console.log("2i: "+cmp(floats.slice(ns * 2, (ns + pns) * 2)));
  console.log("\n"+(end-start)+"ms");
