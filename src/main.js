...
const num = 1000;
let width, height, canvas, ctx, delaunay;
...
fetch("../out/main.wasm").then(function (response) {
  ...
  width = document.getElementById("wrapper").clientWidth * window.devicePixelRatio;
  height = document.getElementById("wrapper").clientHeight * window.devicePixelRatio;
  ...
  let offset = instance.exports.memory.buffer.byteLength;
  // 1ページ分(65536バイト)のメモリを確保する
  instance.exports.memory.grow(Math.ceil((num * (8 + 8 + 4 + 8 + 4 + 8 + 4 + 8 + 4 + 4)) / 65536));
  let offset32 = Math.ceil(offset / 4);
  let pos = new Float32Array(instance.exports.memory.buffer, offset, num * 2);
  let ver = new Float32Array(instance.exports.memory.buffer, offset + num * 8, num * 2);
  let hfeg = new Int32Array(instance.exports.memory.buffer, offset + num * 16, num * 1);
  let tria = new Int32Array(instance.exports.memory.buffer, offset + num * 20, num * 1);

  //console.log(instance.exports.memory, instance.exports.memory.buffer);
  //console.log(instance.exports.memory, instance.exports.memory.buffer);
  for (let i = 0; i < num; i++) {
    pos[i * 2] = Math.random() * width;
    pos[i * 2 + 1] = Math.random() * height;
    ver[i * 2] = Math.random() * 1.4 - 0.7;
    ver[i * 2 + 1] = Math.random() * 1.4 - 0.7;
  }
  //console.log(pos, num, width);

  let points = new Array(num);
  for (let i = 0; i < num; i++) {
    points[i] = [pos[i * 2], pos[i * 2 + 1]];
  }
  delaunay = Delaunator.from(points);
  for (let i = 0; i < delaunay.halfedges.length; i++)
    hfeg[i] = delaunay.halfedges[i];
  for (let i = 0; i < delaunay.triangles.length; i++)
    tria[i] = delaunay.triangles[i];

  // C言語のtest()を実行する
  instance.exports.init(instance.exports.memory.buffer, offset, num, width, height, delaunay.triangles.length);
  let framecount;
  framecount = 0;
  ...
  function frame() {
    ...
    if (framecount == 60) {
      framecount = 0;
      document.getElementById("fps").innerText = 1000 / (performance.now() - prev) * 60;
...
function draw() {
  for (let i = 0; i < num; i++) {
    let vers = Math.sqrt(ver[i * 2 + 0] * ver[i * 2 + 0] + ver[i * 2 + 1] * ver[i * 2 + 1]);
    let normx = ver[i * 2 + 0] / vers * 2.0, normy = ver[i * 2 + 1] / vers * 2.0;
    ctx.beginPath();
    ctx.moveTo(pos[i * 2 + 0] + normx, pos[i * 2 + 1] + normy);
    ctx.lineTo(pos[i * 2 + 0] - normy, pos[i * 2 + 1] + normx);
    ctx.lineTo(pos[i * 2 + 0] + normy, pos[i * 2 + 1] - normx);
    ctx.fill();
  }
}