...
function regressionLib(base64) {
  ...
  const fitRegressionLine = window["fitRegressionLine"] = function(points, __strat, __max) {
    ...
    const useStrategy = __strat|0;
    const maxDegree = typeof __max === "number" && __max > 0 ? __max|0 : 1;
    ...
    if (isBuff || typeof points[0] === "number") {
      ...
      lengthOfPoints = bufferIndex - maxDegree + 1|0;
    } else throw Error("Cannot pass non-array to fitRegressionLine");

    let degreesOfResult = 0;
    if (wasmRegressFunc) {
      degreesOfResult = wasmRegressFunc(useStrategy, maxDegree);
    } else {
      // Much slower fallback without wasm
      
    }
    /*let lastIndex = maxDegree;
    do {
      lastIndex = lastIndex - 1|0;
    } while (viewAsFloat64[0], )*/
    return Array_proto_slice.call(viewAsFloat64, 0, degreesOfResult);// lastIndex+1|0)
  };
  let promise;
  if (wasm) {
    const args = {
      "env": {
        "expandMemory": function(n) {
          memory["grow"](n);
        }
      },
      "js": { "mem": memory }
    ...
    promise.then(function(inst) {promise = null; wasmRegressFunc = inst["exports"]["regress"]});
  }
  fitRegressionLine["onReady"] = function(handle) {
    promise ? promise.finally(handle) : handle();
  };
}
...
fetch(x).then(response => response.arrayBuffer()).then(function(bytes) {
  ...
  regressionLib(asBase64);

  fitRegressionLine.onReady(function() {
    document.getElementById("container").insertAdjacentHTML("beforeend",
      JSON.stringify(fitRegressionLine.regress([[0,0],[1,2],[2,4],[3,3]])));
    document.getElementById("container").insertAdjacentHTML("beforeend","<br /><br />");
    document.getElementById("container").insertAdjacentHTML("beforeend",
      "<sect>" + asBase64 + "</sect>");
  });
  document.getElementById("container").insertAdjacentHTML("beforeend",
    JSON.stringify(fitRegressionLine([[0,0],[1,2],[2,4],[3,3],[4,3],[5,3]])));
