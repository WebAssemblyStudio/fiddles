  ...
  for(var cwn = 0; cwn <= words.length-1; cwn++) { // checks and compiles each word
    ...
    if(currentWord == "if" && command !="comment") {
      ...
      if(command == "if") {
        ...
        var numOfIfConditionStackEnds = 1;
      }
      commandOffset = cwn;
    }
    else if(currentWord == "{" && command !="comment") {
       ...
       */
        ...
        if(currentWord == ")") {
          ...
          if(currentWord == ">"||currentWord == "<"||currentWord == ">="||currentWord == "<="||currentWord == "=="||currentWord == "!=") {
            ...
            if(ifConditionType == "f64") { // it is going to be an f64 check
                toUnshift += "f64.";
            } else { // it is going to be an i32 check
                toUnshift += "i32.";
            }
            ...
            delayedStackOutput.unshift(toUnshift);
          } else if(currentWord == "&&"){
            
          } else if(currentWord == "||"){
            
          } else if(currentWord == "!"){ // only run if whatever is not true
