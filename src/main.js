...
for(var cln = 0; cln <= lines.length-1; cln++) { // checks and compiles each line
  ...
  for(var cwn = 0; cwn <= words.length-1; cwn++) { // checks and compiles each word
    ...
    if(semiForNum) { // MUST BE DONE BEFORE OTHER COMPILATION, INVOLVES CHANGING WORDS
      if (semiForNum == 2 && currentWord != ";") { // DO LAST
        words.splice(cwn, 1);
        cwn -= 1; // solves words array changing position but not cwn
        forOldStackOutput.push(currentWord);
      }
    }
              ...
              if(controlScope[x]!="for"||controlScope[x]!="while") {
                if(controlScope[x] == "if") {
                  ...
                  if(controlScope[x] == "for-start") {
                    for(let x = 0; x <= forOldStackOutput.length-1; x++) {
                      words.push(forOldStackOutput[x]);
                    }
                  }
