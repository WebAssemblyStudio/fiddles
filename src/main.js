...
function add(str,index){
  // add str to data at 'index'
  let offset = instance.exports.data_offset(index);
  let len = instance.exports.data_len();
  data = new Uint32Array(memory.buffer , offset, len);
  for (let i=0; i<str.length; i+=1) {
    data[i] = str.codePointAt(i);
  }
  return true;
}
...
WebAssembly.instantiateStreaming(
  ...
  memory = instance.exports.memory; // le segment de mémoire partagée
  ...
  document.getElementById("container").textContent = add("hello12345",1);
  //document.getElementById("container").textContent = add("2",1);
  offset = instance.exports.data_offset(0);
  len = instance.exports.data_len();
  data = new Uint32Array(memory.buffer , offset, len);
