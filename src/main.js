...
function BytePacker(size)
{
  var c = new ArrayBuffer(size)
  var currentindex = 0;
  this.WriteInt = () => {
...
function Vector2(x,y)
{
  this.x = x;
  this.y = y;
  this.ToPointer = function(buffer) {
    var buff = new Float32Array(buffer,0,8);
    buff.set([x,y]);
    return buff;
  }
}
...
fetch(x).then(response =>
  ...
  var {main,memory,GetArea,Test} = instance.exports;

  var vector2 = new Vector2(3,4);
  var buff = vector2.ToPointer(memory.buffer);
  var result = GetArea(buff.byteOffset)
  console.log(result);
