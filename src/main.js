...
for(var cln = 0; cln <= lines.length-1; cln++) { // checks and compiles each line
  ...
  for(var cwn = 0; cwn <= words.length-1; cwn++) { // checks and compiles each word
     ...
     */
    if(currentWord == "else" && command !="comment") {
      ...
      if(controlScope) {
        console.log(controlScope[controlScope.length-1]);
        if(controlScope == "if") {
     ...
     */
    if(currentWord == "while" && command !="comment") {
      command = "while";
      scope = "";
      controlScope = "while";
      commandOffset = cwn;
     ...
     */
    if(currentWord == "for" && command !="comment") {
      command = "for";
      scope = "";
      controlScope = "for";
      commandOffset = cwn;
        ...
        if(cwn-commandOffset > 2) { // Too many arguments
          ...
          if(cwn-commandOffset == 2) {
              ...
              if(!currentVariable) { // not in a variable, print last thing
                ...
                if(controlScope == "if") { // if statement output works weird
                  if(!options.includes("output-warning")) { // respect compiler options
