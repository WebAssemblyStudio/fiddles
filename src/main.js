fetch('../out/main.wasm').then(response => response.arrayBuffer()).then(bytes => WebAssembly.instantiate(bytes)).then(results => {
  var js = this;
  var wa = results.instance.exports;
...
function test(fn1, fn2, arg) {
  ...
  output += '<tr><td></td><td>' + Math.round((duration1/ duration2) * 100) / 100 + ' x faster</td><td></td></tr>';
