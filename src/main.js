...
for(var cln = 0; cln <= lines.length-1; cln++) { // checks and compiles each line
  ...
  for(var cwn = 0; cwn <= words.length-1; cwn++) { // checks and compiles each word
    ...
    /**
     * for ( initialization ; n to n ; do at end ; ) { }
     * Runs the code inside of the curly brackets while the condition is true.
     * n to n is how you specify how many times it will run. I.e. x to 10 will stop when x is ten.
     * Example:
     ...
     */
      ...
      commandOffset = cwn;
      var semiForCount = 1; // one because 0 returns a falsey value
    }
    // VARIABLES
    /**
     * number x = num1
     ...
     */
        ...
        if(semiForCount) {
          semiForCount += 1;
          if(semiForCount > 4) {
            console.warn("Too many parameters of for statement. Expected only (Initialization) (Condition) (Last) but got more. Line "+cln+" word "+cwn+".")
          }
        }
    }
     ...
     */
      ...
      if(scope == "variable") {
        ...
        if (controlScope[controlScope.length-1] == "for") { // most recent is for
          if(currentWord == ")") {
            if(semiForCount == 4) { // valid
              stackOutput += "(br_if 1 (i32.eqz ("+forData[1]+".eq ("
              if(document.getElementById("compatibility").checked == true) { // COMPATIBILITY
                stackOutput += "get_local";
              } else {
                stackOutput += "local.get";
              }
              stackOutput += " $"+forData[0]+" )("// +forData[1].const 5 ))))\n
              if(!isNaN(parseInt(forData[2].substring(0,1)))) { // is a constant
                stackOutput += forData[1]+".const "+forData[2]+" ))))\n";
              } else { // variable (already validated)
                if(document.getElementById("compatibility").checked == true) { // COMPATIBILITY
                  stackOutput += "get_local";
                } else {
                  stackOutput += "local.get";
            ...
            } else { //invalid not enough params
              console.error("Not enough parameters for for loop on line "+cln+" word "+cwn+".");
            }
          } else if(currentWord == "(") {
            var forData = ["","",""]; // variable, equal type, equal
          } else {
            if(words[cwn+1] == "to") { // if the next word is to
              if(!isNaN(parseInt(currentWord.substring(0,1)))) { // is a constant
                console.error("Left sign of to operator must be a variable. Line "+cln+" word "+cwn+".");
              } else {
                if(variables.includes(currentWord)) {
                  forData[0] = currentWord; // initial
                } else {
                  console.error(currentWord+" is not a variable. Declare it before using it. Line "+cln+" word "+cwn+".");
                }
              }
            }
            if(words[cwn-1] == "to") { // if the previous word is to
            if(!isNaN(parseInt(currentWord.substring(0,1)))) { // is a constant
