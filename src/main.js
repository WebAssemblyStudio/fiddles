...
numOfOpenControls = 0; // makes sure not closing statements too early
...
for(var cln = 0; cln <= lines.length-1; cln++) { // checks and compiles each line
  ...
  for(var cwn = 0; cwn <= words.length-1; cwn++) { // checks and compiles each word
     ...
     */
      ...
      if(controlScope) {
        if(controlScope[controlScope.length-1] == "if") { // if most recent is if
          if(words[cwn+1] != "else") { // this is bad because no spaces but also I think this is the best way to do it come back to do come do later
            for(let x = 0; x<=controlScope.length-1; x++) { // for everything in controlscope
              if(controlScope[x]!="for"||controlScope[x]!="while") {
                if(controlScope[x] == "if") {
                  ...
                  stackOutput += "(; not if, "+controlScope[x]+" ;) )\n";
                }
        ...
        if(controlScope[controlScope.length-1] == "else") {
          stackOutput += " )\n)\n"; // ends the else and the if.
          command = "";
        }
        numOfOpenControls -= 1;
        if(numOfOpenControls == 0) {
          controlScope = "";
        }
      } else {
     ...
     */
      ...
      if(!controlScope.includes("if")) {
        command = "output";
        scope = "output";
        commandOffset = cwn;
      } else {
        if(!options.includes("output-warning")) { // respect compiler options
          console.warn("An output specified in an if statement will not run inside of the if statement. Line "+cln+" word "+cwn+".");
        }
      }
     ...
     */
      ...
      if(scope == "variable") {
        if(command == "variable-set") { // not a decimal
          ...
          commandOffset = cwn;*/ // come do later come back to
        }
       ...
       */
        ...
        if(currentWord == ")") {
          ...
          if(currentWord == ">"||currentWord == "<"||currentWord == ">="||currentWord == "<="||currentWord == "=="||currentWord == "!=") {
            ...
            if(currentWord == "==") { // EQUAL
              toUnshift += "eq";
            } else if(currentWord == "!=") { // NOT EQUAL
              toUnshift += "ne";
            } else if(currentWord == ">") { // GREATER THAN
              toUnshift += "gt_s";
            } else if(currentWord == "<") { // LESS THAN
              toUnshift += "lt_s";
            } else if(currentWord == ">=") { // GREATER THAN OR EQUAL TO
              toUnshift += "ge_s";
            } else if(currentWord == "<=") { // LESS THAN OR EQUAL TO
              toUnshift += "le_s";
            } else {
              console.error(currentWord+" is not an operator. Line "+cln+" word "+cwn+".");
            }
