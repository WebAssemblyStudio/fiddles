...
fetch(x).then(response =>
  ...
  WebAssembly.instantiate(bytes, {
    env: {
      ...
      _Z9printTextPc: function(addr){
        console.log("Print text ptr: " + addr);
      },

      _Z9mapMemoryjsj: function(addr, size){
          console.log("Map mem: " + addr + ", " + size);
      },

      _Z8printInti: function(num){
          console.log("Print int: " + num);
      },

      _Z10printFloatf: function(num){
          console.log("Print float: " + num);
      },
    }
  })
).then(results => {
