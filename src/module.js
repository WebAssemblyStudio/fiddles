...
function drawFrame(frame) {
    ...
    */
    ...
    const time = frame.frameNumber;
    function dl(time) {
        var t = Math.floor(Math.sin(time/40)*20)
        frame.color = (0xff << 16) | ((time*4%255) << 8) | ((time*10)%255);
        frame.drawLine(0, frame.height/2+t, frame.width, frame.height/2+t)
    }
    dl(time)
    dl(frame.frameNumber+40)
    dl(frame.frameNumber+80)
    dl(frame.frameNumber+120)

    function dl2(time) {
        var t = Math.floor(Math.sin(time/30)*20)
        frame.color = 0xff0000;
        frame.drawLine(frame.width/2+t, 0, frame.width/2+t, frame.height)      
    }
    dl2(time)
    // dl2(time+40)
    dl2(time+80)
