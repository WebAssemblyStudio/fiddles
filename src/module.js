...
function drawFrame(frame) {
  ...
  if (frame.frameNumber % 1000 !== 0) return; // move slow
  const count = frame.frameNumber / 1000;

  let offset_x = count % (frame.width - WIDTH); // how far to push logo over
  let offset_y = 0;
  ...
  for (let y = 0; y < frame.height; y++) {
    for (let x = 0; x < frame.width; x++) {
      ...
      if (y < HEIGHT + offset_y && y > offset_y && x < WIDTH + offset_x && x > offset_x) {
        color = IMAGE[WIDTH * (y - offset_y) + x - offset_x];
      }
