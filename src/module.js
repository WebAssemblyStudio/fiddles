...
let checkY = (lines, y) => lines.find((line) => line[1] === y)

let lines = []
function drawFrame(frame, i) {
  if (i % 5 === 0) {
    let randomHeight = randomNumber(frame.height / 2, frame.height)
    let randomWidth = randomNumber(0, frame.width)
    // dont add to a line that has one already
    if (!checkY(lines, randomWidth)) {
      lines.push([randomWidth, randomHeight, COLORS[randomNumber(0, COLORS.length)]])
    }
  }
  ...
  if (i % 2 === 0) {
    lines = lines.reduce((acc, value, index) => {
      ...
      if(newValue > 0) {
        acc.push([ newValue, value[1], value[2] ])
      }
      return acc
    }, [])
  }
  ...
  for (let line of lines) {
    frame.color = line[2]
    frame.drawLine(line[1], 0, line[1], line[0])
  }
}
