...
function drawFrame(frame) {
    ...
    let color;
    if (fnforcolor == 0) {
      color = colorRotation.shift();
      fnforcolor = frame.width / 2
    } else {
      color = colorRotation[0];
    }
    //let framenumber = 2;
    ...
    for(let y = 0; y < frame.height; y++) {
        for(let x=0; x<frame.width; x++) {
            //even or odd
            if((x > (frame.width / 2 - framenumber)) && (x < frame.width / 2 + framenumber)) {
                frame.color = color;
                frame.setPixel(x,y);
            } else {
                frame.color = WHITE
