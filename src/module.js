...
const CONT_COLORS = [0xc6e48b, 0x7bc96f, 0x239a3b, 0x196127];
...
function drawFrame(frame) {
  ...
  const should_change = true;
  const num_bars = 7; // TODO determine
  if (should_change) {
    CONTRIBUTIONS = []
    for (let i = 0; i < frame.width * num_bars; i++) {
      const rand = Math.floor(Math.random() * CONT_COLORS.length)
      CONTRIBUTIONS.push(CONT_COLORS[rand]);
    }
  }
  ...
  for (let y = 0; y < num_bars; y++) {
    for (let x = 0; x < frame.width; x++) {
      frame.color = CONTRIBUTIONS[y * frame.width + x];
