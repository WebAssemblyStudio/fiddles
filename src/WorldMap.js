class WorldMap{
  ...
  setTesting(x,z){
    this.drawPixel(x,z,255,255,this.getBlueValue(x,z),255);
  }
  undoTesting(x,z){
    this.drawPixel(x,z,0,0,this.getBlueValue(x,z),255);
  }
  getBlueValue(x,z){
    return this.ctx.getImageData(0, 0, this.radius*2, this.radius*2).data[4*(2*z*this.radius + x)+2];
  }
  getNextChunk(){
    var data = this.ctx.getImageData(0, 0, this.radius*2, this.radius*2).data;
    var i_best = -1;
    var b_best = -1;
    ...
    i_best = i_best/4;
    var x = i_best%(this.radius*2);
    var y = (i_best-x)/(this.radius*2);
