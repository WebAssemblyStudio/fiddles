async function showPreview(frames, loop, fps) {
  ...
  //generator.next();
  var interval = setInterval(generator.next.bind(generator), 1000 / fps);
}
...
function* showFrame(rows, frames, loop) {
  ...
  while (true) {
    i++;
    console.log(JSON.stringify({frame: i, length: frames.length}));
    if (i === frames.length) {
      if (!loop) {
        return;
      }
