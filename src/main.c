  ...
  if(1) {
    ...
    if(totalGs < 100) {
      ...
      int yE = min(sHeight, yEnd+1);
			for(int y=yE; y>=yS; --y) {   // NOTE: yes yE and yS are reversed for easier comprehension
      //for(int y=sHeight; y>=0; --y) {
          ...
          //int thresh_off = (int)(log(abs(x-mouseX) + abs(y-mouseY))*2)*sensitivity;
					int thresh_off = logVals2[abs(x-mouseX) + abs(y-mouseY)];
    ...
    if(maskChanges == 0) {
      ...
      for(int y=yS; y<yE; ++y) {			//for(int y=0; y<height; ++y) {
        ...
        for(int x=0; x<width; ++x, ++idx) {			//for(int x=0; x<width; ++x, ++idx) {
          if(!mmask[idx]) {
            float stdDev_off1 = logVals[abs(x-mouseX) + abs(y-mouseY)];   //float stdDev_off1 = log(abs(x-mouseX) + abs(y-mouseY))*sensitivity;
            ...
            int moff = (y > 0) ? off : 0;
            if(expand < 2) {
              if(abs(data[idx]-gCutoff) > stdDevG+stdDev_off1 && (mmask[idx+p1] || mmask[idx+poff])) {
                maskChanges[idx] = 1;
                //repeat = 1;
              } else if(abs(data[idx]-gCutoff) > stdDevG+stdDev_off1 && (mmask[idx-m1] || mmask[idx-moff])) {
                maskChanges[idx] = 1;
                //repeat = 1;
              }
            } else {
              unsigned char d_idx = data[idx];
              if((abs(d_idx-gCutoff) > stdDevG+stdDev_off1 || abs(d_idx - objEstG)*4 < stdDevG2-stdDev_off1) && abs(d_idx - objEstG) < stdDevG2+objBGDiff-stdDev_off1 && (mmask[idx+p1] || mmask[idx+poff])) {		// check right and down
                maskChanges[idx] = 1;
                //repeat = 1;
              } else if((abs(d_idx-gCutoff) > stdDevG+stdDev_off1 || abs(d_idx - objEstG)*4 < stdDevG2-stdDev_off1) && abs(d_idx - objEstG) < stdDevG2+objBGDiff-stdDev_off1 && (mmask[idx-m1] || mmask[idx-moff])) {	// check left and up
                maskChanges[idx] = 1;
                //repeat = 1;
              }
            }
