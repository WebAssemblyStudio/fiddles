...
//			for(int x = sWidth; x--; ++idx) {
        for(int x = 0; x<width; ++x, ++idx) {
          ...
          int thresh_off = (int)(log(abs(x-mouseX) + abs(y-mouseY))*2)*10;
          thresh_off -= 2;
...
//      printf("for loop 9\n");
      ...
      for(int y=0; y<height; ++y) {
        ...
        for(int x=0; x<width; ++x, ++idx, --idx2) {
          float stdDev_off1 = log(abs(x-mouseX) + abs(y-mouseY))*10;
          float stdDev_off2 = log(abs((sWidth-x)-mouseX) + abs(yp1-mouseY))*10;
	...
	for(int iter = 0; iter < 1; ++iter) {
		...
		for(int y = 0; y < sHeight-1; ++y) {
          ...
          int p8 = min(sWidth-x, 8);
				...
				if(mmask[idx+off] == 0) {
          int poff8 = min(sHeight-y, 8)*8;
					if(mmask[idx+off*2] || mmask[idx+poff8]) {
						mmask[idx+off] = 255;
					}
				}
			}
...
//    printf("for loop 12\n");
		for(int y = sHeight-1; y > 0; --y) {
			int idx = y * off + sWidth2;
			for(int x = sWidth2; x--; --idx) {
				if(!mmask[idx]) {
					continue;
				}
				if(mmask[idx-1] == 0) {
          int m8 = min(sWidth-x, 8);
					if(mmask[idx-2] || mmask[idx-m8]) {
						mmask[idx-1] = 255;
					}
				}
				if(mmask[idx-off] == 0) {
          int moff8 = min(sHeight-y, 8)*off;
