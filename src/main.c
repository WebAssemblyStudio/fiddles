...
void move(){
  ...
  switch(direction){
    case 'N':if(color==1){direction='E';}else{direction='W';};
      break;
    case 'S':if(color==1){direction='W';}else{direction='E';};
      break;
    case 'E':if(color==1){direction='S';}else{direction='N';};
      break;
    case 'W':if(color==1){direction='N';}else{direction='S';};
      break;
   }

  //test out of bounds
  switch(direction){
    case 'N':if(positionX - 1 >=0){positionX-=1;};
      break;
    case 'S':if(positionX + 1 < SIZE){positionX+=1;};
      break;
    case 'E':if(positionY + 1 < SIZE){positionY+=1;};
      break;
    case 'W':if(positionY - 1 >= 0){positionY-=1;};
      break;
  }
