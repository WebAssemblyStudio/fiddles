...
void convert(void* p, int size, int bits) {
  ...
  if(bits == 8){
    ...
    for(int i=0;i<size/4;i++){
      if(src[index] >= 0){
        dst[index] = (int)(src[index] * 32767);                      
      }else{
        dst[index] = (int)(src[index] * 32768);
      }        
      index++;
    }   
  }else if(bits == 24){ 
    for(int i=0;i<size/4;i++){
      if(src[index] >= 0){
        dst[index] = (int)(src[index] * 8388607);                      
      }else{
        dst[index] = (int)(src[index] * 8388608);
      }        
      index++;
    }  
  }else if(bits == 32){ 
    for(int i=0;i<size/4;i++){
      if(src[index] >= 0){
        dst[index] = (int)(src[index] * 2147483647);                      
      }else{
        dst[index] = (int)(src[index] * 2147483648);
      }        
      index++;
    }         
  } 
