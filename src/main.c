...
WASM_EXPORT
void avanceFourmi() { // Avance la fourmi d'un pas
  if(rotation == 0) {
    posx--;
  } else if (rotation == 1) {
    posy--;
...
WASM_EXPORT
void update() {
  while(posx > 0 && posx < SIZE && posy > 0 && posy < SIZE) { // Tant qu'on ne sort pas de l'image
    updateMouvementFourmi(); // On mets à jour la fourmi
  }
}