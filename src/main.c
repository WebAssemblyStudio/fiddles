...
char *intToStr(int value){ char *text = (char *)malloc(1024);char *p = text+100;(*p--) = 0;while(value > 10){int vl = (int)(value/10); int rest = value - ((vl)*10);(*p--) = '0' + rest; value=vl; }(*p) = '0' + value; return p;}
void intToConsole(int value){ char *text = (char *)malloc(1024);char *p = text+100;(*p--) = 0;while(value > 10){int vl = (int)(value/10); int rest = value - ((vl)*10);(*p--) = '0' + rest; value=vl; }(*p) = '0' + value; x.data[x.cnt++]=p; x.len+=(p-text);}
...
char *join( key xxe, char *by){int b = (*by) ? strlen(by) : 0; char *bs;int cn=0; char* ret = (char*)malloc(xxe->len+2+b*xxe->cnt);char *s, *d = ret;   while(cn<(xxe->cnt-1)) {    s=getDAT(cn++,xxe); int dlen=strlen(s); strCopySized(d,s,dlen); d+=dlen;  if (b){bs = by; strCopySized(d,bs,b) d+=b;}}   s=getDAT(cn++,xxe); int dl=strlen(s); strCopySized(d,s,dl); d+=dl; d[0]="\0"; return ret;}
...
#define vk_label 252
#define vk_list 253
#define vk_getter 254
#define vk_setter 255
