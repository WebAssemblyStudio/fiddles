...
int caller_is_owner() {
  ...
  owner = sdk_storage_get((uint8_t *)OWNER, sizeof(OWNER));
...
int set_owner(address owner) {
  ...
  sdk_storage_set((uint8_t *)OWNER, sizeof(OWNER), owner, ADDR_SIZE);
  return 0;
}
...
int pause() {
  ...
  uint8_t pause = 1;
  sdk_storage_set((uint8_t *)IS_PAUSE, sizeof(IS_PAUSE), &pause, 1);
  return 0;
}
...
int unpause() {
  ...
  uint8_t unpause = 0;
  sdk_storage_set((uint8_t *)IS_PAUSE, sizeof(IS_PAUSE), &unpause, 1);
  return 0;
}

int is_pausing() {
  return from_bytes(sdk_storage_get((uint8_t *)IS_PAUSE, sizeof(IS_PAUSE)));
}
...
int set_owner_to_creator() {
  address creator = (address) malloc(ADDR_SIZE * sizeof(byte_t));
  chain_get_creator(creator);
  sdk_storage_set((uint8_t *)OWNER, sizeof(OWNER), creator, ADDR_SIZE);
  free(creator);
  return 0;
}
