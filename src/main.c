...
WASM_EXPORT
unsigned char* autoHighlight(unsigned char* data, unsigned char* mmask, unsigned char *nnimg, int width, int height, int expand, int mouseX, int mouseY, int useNNImg) {

	expand = expand % 4;	// 1+2 expand edges if new pixel is different from object, 3+4 expand edges if new pixel is similar to object (also in 3 expand into lighter areas)
	...
	if(!logVals || !logVals2) {
		...
		int wh = 500+500;
		logVals  = logVals  ? logVals  : (int*)malloc(wh*sizeof(int));
		logVals2 = logVals2 ? logVals2 : (int*)malloc(wh*sizeof(int));
		logVals[0]  = 0;
		logVals2[0] = 0;
		for(int i=1; i<wh; ++i) {
			float v = log(i);
	...
	if(maskChanges == 0) {
		maskChanges = (unsigned char *)malloc(500*500);
		memset(maskChanges, 0, 500*500);
	}
	...
	for(int rpt=0; rpt<rptCnt; ++rpt) {
		...
		for(int i=0; i<256; ++i) {
			int prevG = gList[i];
			int minus = (int)(gListInner[i]*scale);
			gList[i] = max(gList[i]-(int)(gListInner[i]*scale), 0);
			totalGs -= prevG - gList[i];
		}
		...
		for(int iter=0; iter<3 && expand==3; ++iter) {
			...
			for(int y=yE; y>=yS; --y) {	 // NOTE: yes yE and yS are reversed for easier comprehension		//for(int y=sHeight; y>=0; --y) {
				...
				for(int x=sWidth; x>=0; --x, --idx) {
					...
					//int thresh_off = logVals2[abs(x-mouseX) + yMAbs];
					thresh_off = max(thresh_off-2, 0);
					int nn_off = (nnimg[idx] ? 0 : -4) * useNNImg;
					int threshold = -4 - thresh_off + nn_off;

					int d_idx = data[idx];
					//int thresh_min_samp = threshold - d_idx;
					if(d_idx < gCutoff + nn_off) {	// if the current pixel is darker than the median brightness
			...
			for(int y=yS; y<yE; ++y) {			//for(int y=1; y<height; ++y) {
				int idx = y*off;
				...
				for(int x=0; x<width; ++x, ++idx) {
					...
					thresh_off = max(thresh_off-2, 0);
					int nn_off = (nnimg[idx] ? 0 : -4) * useNNImg;
					int threshold = -4 - thresh_off + nn_off;		// -2??
		...
		for(int z=0; z<1; ++z) {
			...
			for(int y=yS; y<yE; ++y) {			//for(int y=0; y<height; ++y) {
				...
				for(int x=0; x<width; ++x, ++idx) {			//for(int x=0; x<width; ++x, ++idx) {
					if(!mmask[idx]) {
						// TODO: determine if we should make stdDev_off and logVals a float
						int stdDev_off = logVals_pY[abs(x-mouseX)];	 //float stdDev_off1 = logVals[abs(x-mouseX) + abs(y-mouseY)];	 //float stdDev_off1 = log(abs(x-mouseX) + abs(y-mouseY))*sensitivity;
						//int stdDev_off = logVals[abs(x-mouseX) + yMAbs];
						int nn_off = (nnimg[idx] ? nnimg[idx] : -15) * useNNImg;
						//stdDev_off = max(stdDev_off - .5, 0) - nnimg[idx];
						stdDev_off = max(stdDev_off - .5, 0) - nn_off;
						...
						int moff = (y > 0) ? off : 0;
						if(expand < 2) {
							if(abs(data[idx]-gCutoff) > stdDevG+stdDev_off) {
								if(mmask[idx+p1] || mmask[idx+poff]) {
									//maskChanges[idx] = 1;
									//repeat = 1;
								} else if(mmask[idx-m1] || mmask[idx-moff]) {
									//maskChanges[idx] = 1;
									//repeat = 1;
								}
							}
							...
							// ((is current pixel very different from median border color?) || (is current pixel very similar to object color)) && (is current pixel similar enough to object color)
							if((abs(d_idx-gCutoff) > stdDevG+stdDev_off || abs(d_idx - objEstG)*4 < stdDevG2-stdDev_off) && abs(d_idx - objEstG) < stdDevG2+objBGDiff-stdDev_off) {
								if(mmask[idx+p1] || mmask[idx+poff]) {		// check right and down
									maskChanges[idx] = 1;
									//repeat = 1;
									//printf("a1 d_idx: %d  gCutoff: %d  stdDevG: %d  stdDev_off: %d  nn_off: %d  objEstG: %d  stdDevG2: %d  objBGDiff: %d\n", d_idx, gCutoff, (int)stdDevG, stdDev_off, nn_off, objEstG, (int)stdDevG2, objBGDiff);
								} else if(mmask[idx-m1] || mmask[idx-moff]) {	// check left and up
									maskChanges[idx] = 1;
									//repeat = 1;
									//printf("a2 d_idx: %d  gCutoff: %d  stdDevG: %d  stdDev_off: %d  nn_off: %d  objEstG: %d  stdDevG2: %d  objBGDiff: %d\n", d_idx, gCutoff, (int)stdDevG, stdDev_off, nn_off, objEstG, (int)stdDevG2, objBGDiff);
								}
							}
						}
					}
			...
			if(expand == 0) {
				...
				for(int y=yS; y<yE; ++y) {		//for(int y=0; y<height; ++y) {
					...
					for(int x=0; x<width; ++x, ++idx) {
						unsigned char d_idx = data[idx];
						//int nn_off = (nnimg[idx] ? nnimg[idx] : -4) * useNNImg;
						int nn_off = (nnimg[idx] ? nnimg[idx] : -20) * useNNImg;
					//	if(mmask[idx] && abs(d_idx-objEstG) < stdDevG2+nnimg[idx]) { // TODO: determine if putting the second part of this if statement here makes this faster
						if(mmask[idx] && abs(d_idx-objEstG) < stdDevG2+nn_off) { // TODO: determine if putting the second part of this if statement here makes this faster
						//if(mmask[idx]) {
							//printf("in! x: %d  y: %d\n", x, y);
							int p1 = (x < sWidth) ? 1 : 0;
							int m1 = (x > 1) ? 1 : 0;
							...
							//float stdDevG_nnimg = stdDevG-nnimg[idx];
							float stdDevG_nnimg = stdDevG - nn_off;

							if(!mmask[idx+p1] && abs(d_idx-data[idx+p1]) > stdDevG_nnimg) {
								maskChanges[idx+p1] = 1;
								//printf("a1 x: %d  y: %d  idx changed: %d\n", x, y, idx+p1);
							//} else if(!mmask[idx+p1]) {
								//printf("f1 x: %d  y: %d  d_idx: %d  data[idx+p1]: %d  stdDevG_nn_img: %d  nn_off %d  stdDevG: %d\n", x, y, d_idx, data[idx+p1], (int)stdDevG_nnimg, nn_off, (int)stdDevG);
							}
							if(!mmask[idx-m1] && abs(d_idx-data[idx-m1]) > stdDevG_nnimg) {
								maskChanges[idx-m1] = 1;
								//printf("a2 x: %d  y: %d  idx changed: %d\n", x, y, idx-m1);
							}
							if(!mmask[idx+p1off] && abs(d_idx-data[idx+p1off]) > stdDevG_nnimg) {
								maskChanges[idx+p1off] = 1;
								//printf("a3 x: %d  y: %d  idx changed: %d\n", x, y, idx+p1off);
							}
							if(!mmask[idx-m1off] && abs(d_idx-data[idx-m1off]) > stdDevG_nnimg) {
								maskChanges[idx-m1off] = 1;
								//printf("a4 x: %d  y: %d  idx changed: %d\n", x, y, idx-m1off);
							}
