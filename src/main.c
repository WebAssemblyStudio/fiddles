...
int caller_is_owner() {
  ...
  owner = sdk_storage_get((byte_t*)OWNER, OWNER_KEY_SIZE);
...
int set_owner(address add) {
  ...
  sdk_storage_set((byte_t*)OWNER, OWNER_KEY_SIZE, add, ADDR_SIZE);
  return 0;
}
...
int pause() {
  ...
  uint8_t pause = 1;
  sdk_storage_set((byte_t*)IS_PAUSE, IS_PAUSE_KEY_SIZE, &pause, 1);
  return 0;
}
...
int unpause() {
  ...
  uint8_t unpause = 0;
  sdk_storage_set((byte_t*)IS_PAUSE, IS_PAUSE_KEY_SIZE, &unpause, 1);
  return 0;
}

int is_pausing() {
  return from_bytes(sdk_storage_get((byte_t*)IS_PAUSE, IS_PAUSE_KEY_SIZE));
}
...
int set_owner_to_creator() {
  byte_t* creator = (byte_t*) malloc(ADDR_SIZE * sizeof(byte_t));
  chain_get_creator(creator);
  sdk_storage_set((byte_t*)OWNER, OWNER_KEY_SIZE, creator, ADDR_SIZE);
  return 0;
}
