...
extern	int			GetPaintMessageFlag(			void ) ;												// WM_PAINT メッセージが来たかどうかを取得する(戻り値  TRUE:WM_PAINTメッセージが来た(一度取得すると以後、再び WM_PAINTメッセージが来るまで FALSE が返ってく���よ���になる)  FALSE:WM_PAINT メッセージは来ていない)
...
extern	int			DragFileInfoClear(			void ) ;																		// メイン���インドウへドラッグ＆ドロップされたファイルの情報をリセットする
...
extern	int			InsertMenuItem_Name(				const TCHAR *ItemName,                        const TCHAR *NewItemName                           ) ;				// 指定の項目と、��定の項目の一つ上の項目との間に新しい項目を追加する
...
extern	int			GetUseDirect3D11FeatureLevel(					void ) ;										// 使用している Direct3D11 の FeatureLevel ( DX_DIRECT3D_11_FEATURE_LEVEL_9_1 等 )を���得する( 戻り値　-1：エラー　-1以外：Feature Level )
...
extern	int			PlayMusicByResource(				const TCHAR *ResourceName,                            const TCHAR *ResourceType,                            int PlayType ) ;	// リソース���らＭＩＤＩファイルを読み込んで演奏する
