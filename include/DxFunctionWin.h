...
extern	int			GetAlwaysRunFlag(				void ) ;												// メインウインドウが非アクティブになっても処理を実行し続け��かどうかの設定を取得する( TRUE:実行する  FALSE:停止する )
...
extern	int			GetUseWindowRgnFlag(			void ) ;												// リージョンを使っている���どうかを取得する
...
extern	int			GetNotDrawFlag(					void ) ;												// ＤＸライブラリの描画機能を使うかどうかの設定���取得する
...
extern	int			SetUserWindowMessageProcessDXLibFlag(	int Flag ) ;																		// SetUseWindow で設定したウインドウのメッセージループ処理をＤ���ライブラリで行うかどうかを設定する( TRUE:ＤＸライブラリで行う( デフォルト )  FALSE:ＤＸライブラリでは行わない )
...
extern	int			SetMenuItemMark(					const TCHAR *ItemName,                        int ItemID, int Mark ) ;												// メニューの項目にチェックマークやラジオボタンを表示するかどうかを設定する( Mark:設定するマーク( MENUITEM_MARK_NONE 等( 解説は #define の定義を参照してください )    ItemName と ItemID については関数 DeleteMenuItem の注釈を参照してくだ��い ) )
...
extern	int			DeleteMenuItemAll(					void ) ;																											// メニューの全ての選��項目を削除する
...
extern	int			SetUseTSFFlag( int UseFlag ) ;											// ＩＭＥの漢字変換候補表示���処理に TSF を使用するかどうかを設定する( TRUE:使用する( デフォルト )  FALSE:使用しない )
...
// 情報取得系関数
extern	const void*	GetDSoundObj(                        void ) ;	/* 戻り値を IDirectSound * にキャストして下さい */																					// ＤＸライブラリが使用している DirectSound オブジェクトを取得す���
