...
extern	int			GetPaintMessageFlag(			void ) ;												// WM_PAINT メッセージが来たかどうかを取得する(戻り値  TRUE:WM_PAINTメッセージが来た(一度取得すると以後、再び WM_PAINTメッセージが来るまで FALSE が返ってくるようになる)  FALSE:WM_PAINT メッセ���ジは来ていない)
...
extern	int			DragFileInfoClear(			void ) ;																		// メインウインドウへドラッグ＆ドロップされ���ファイルの情報をリセットする
...
extern	int			InsertMenuLine_Name(				const TCHAR *ItemName                        ) ;																	// 指定の項目と、指定の項��の一つ上の項目との間に区切り線を追加する
...
extern	int			SetUseDirect3D9Ex(								int Flag ) ;									// Vista以降の環境で Direct3D9Ex を使用するかどうかを設定する( TRUE:使用する( デフォルト )  FALSE:使用しない )( DxLib_Init の前に呼��必要があります )
...
extern	HANDLE		AddFontFile(					const TCHAR *FontFilePath                            ) ;										// 指定のフォントファイルをシステムに追加する( 戻り値  NULL:失敗  NULL以外:フォントハンドル( WindowsOS のものなので、ＤＸライブラリのフ��ントハンドルとは別物です ) )
...
extern	int			UpdateLayerdWindowForPremultipliedAlphaBaseImage(     const BASEIMAGE *BaseImage ) ;																										// 乗算済みアルファの基本イメージデータを使用して UpdateLayerdWindow ���行う
...
// 設定関係関数
extern	int			SetUseSoftwareMixingSoundFlag(       int Flag ) ;																																	// サウンドの処理をソフトウエアで行うかどうかを設定する( TRUE:ソフ���ウエア  FALSE:ハードウエア( デフォルト ) )
