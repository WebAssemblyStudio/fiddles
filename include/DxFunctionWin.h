...
extern	int			SetWindowSizeChangeEnableFlag(			int Flag, int FitScreen = TRUE ) ;													// メインウインドウの端を掴んでウインドウのサイズを変更できるようにするかどうかの設定を行う( Flag:変更を可能にするかどうか( TRUE:可能にする  FALSE:不可能にする( デフォルト ) )  FitScreen:ウインドウのクライアント領域に画面をフィットさせる(拡大させる)かどうか  TRUE:フィット��せる  FALSE:フィットさせない )
...
extern	int			DeleteMenuItemAll(					void ) ;																											// メニューの全ての���択項目を削除する
...
extern	int			UpdateLayerdWindowForSoftImageRect(                   int SIHandle, int x1, int y1, int x2, int y2 ) ;				// ソフトウエアイメージハンドルの指定の���囲を使用して UpdateLayerdWindow を行う
