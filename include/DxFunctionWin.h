...
extern	int			SetMenuItemEnable_ID(				int ItemID, int EnableFlag ) ;																						// メニ���ーの項目を選択出来るかどうかを設定する( EnableFlag　1:選択できる  0:選択できない )
...
extern	int			SetAutoMenuDisplayFlag(				int Flag ) ;																										// フルスクリーン時にメニューを自動で表示したり非表示にしたりするかどうかのフラグを��ットする
...
// デスクトップキャプチャ
extern	int			GetDesktopScreenBaseImage( int x1, int y1, int x2, int y2, BASEIMAGE *BaseImage, int DestX, int DestY ) ;						// デスクトップの指定の領域を基本イメージデー��に転送する
...
// 情報取得系関数
extern	const void*	GetDSoundObj(                        void ) ;	/* 戻り値を IDirectSound * にキャストして下さい */																					// Ｄ��ライブラリが使用している DirectSound オブジェクトを取得する
