...
extern	int			DragFileInfoClear(			void ) ;																		// メインウイ��ドウへドラッグ＆ドロップされたファイルの情報をリセットする
...
extern	int			InsertMenuLine_ID(					int ItemID, int NewItemID ) ;																						// 指定の項目と、指定の項目の一つ上��項目との間に区切り線を追加する
...
extern	int			SetUseXboxControllerDirectInputFlag(	int Flag ) ;															// Xbox360コントローラや Xbox Oneコントローラを DirectInputコントローラとしても検出するかどうかを設定する( TRUE:DirectInputコントロー��としても検出する  FALSE:DirectInputコントローラとしては検出しない(デフォルト) )、DxLib_Init の呼び出し前でのみ実行可能
...
extern	int			SetDrawScreen_ID3D11RenderTargetView(			const void *pID3D11RenderTargetView, const void *pID3D11DepthStencilView = NULL ) ;		// 指定の ID3D11RenderTargetView を描画対象にする( pID3D11DepthStencilView が NULL の場合はデフォルト���深度ステンシルバッファを使用する )
