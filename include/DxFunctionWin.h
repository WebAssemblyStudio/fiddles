// -------------------------------------------------------------------------------
// 
// 		ＤＸライブ��リ		Windows専用関数プロトタイプ宣言用ヘッダファイル
// 
// 				Ver 3.20f
// 
// -------------------------------------------------------------------------------
...
extern	int			SetAutoMenuDisplayFlag(				int Flag ) ;																										// フルスクリーン時にメニューを自動で表示したり非表���にしたりするかどうかのフラグをセットする
...
extern	const void*	GetGraphID3D11RenderTargetView(	int GrHandle ) ;												// グラフィックハンドルが持つ ID3D11RenderTargetView を取得する( Direct3D11 を使用していて、且つ MakeScreen で作成したグラフィックハンドルでの��有効 )( 戻り値を ID3D11RenderTargetView * にキャストしてください )
...
// 設定関係関数
extern	int			SetUseSoftwareMixingSoundFlag(       int Flag ) ;																																	// サウンドの処理をソフトウエアで行うかどうかを設定する( TRUE:ソフトウエア  FALSE:ハードウ��ア( デフォルト ) )
