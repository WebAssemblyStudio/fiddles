...
extern	int			SetWindowZOrder(						int ZType /* = DX_WIN_ZTYPE_TOP 等 */ , int WindowActivateFlag = TRUE ) ;			// メインウインドウ���奥行き位置を変更する
...
extern	int			GetDragFileNum(				void ) ;																		// メインウインドウへドラッグ＆ドロップされたファイル��数を取得する
...
extern	int			AddMenuLine_NameWithStrLen(			const TCHAR *ParentItemName, size_t ParentItemNameLength ) ;														// メニューのリストに区切り線を追加する( ParentItemName:���切り線を付ける項目リストの親の名前、リストの末端に区切り線を追加します )
...
extern	int 		LoadMenuResource(					int MenuResourceID ) ;																								// メニ���ーをリソースから読み込む
...
extern	int			PlayMusicByResourceWithStrLen(		const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int PlayType ) ;	// リソースからＭＩＤＩファイ���を読み込んで演奏する
