...
extern	int			GetWindowModeFlag(				void ) ;												// ウインドウモードで起動し���いるかどうかを取得する( 戻り値  TRUE:ウインドウモード  FALSE:フルスクリーンモード )
...
extern	int			SetWindowRgnGraph(			 const TCHAR *FileName                        ) ;								// 任意の画像ファイル��らＲＧＮをセットする
...
extern	int			SetToolBarButtonState(		int ID , int State /* TOOLBUTTON_STATE_ENABLE 等 */ ) ;																	// ツールバーのボタンの状態を設定する( ID:AddToolBarButtonで設定したボ���ンの識別番号　State:設定する状態( TOOLBUTTON_STATE_ENABLE 等( 解説は #define の定義を参照してください ) )
...
extern	int			AddMenuItem_Name(					const TCHAR *ParentItemName,                              const TCHAR *NewItemName                           ) ;	// メニューに選択項目を追加する( ParentItemName:親となる項目の名前、親が持つリストの末���に新しい項目を追加します  NewItemName:新しい項目の名前 )
...
extern	int			CheckMenuItemSelect_NameWithStrLen(	const TCHAR *ItemName, size_t ItemNameLength ) ;																	// メニューが選択されたかどうかを取得する( 戻り��　0:選択されていない  1:選択された )
...
// 画面関係関数
extern	int			BltBackScreenToWindow(			HWND Window, int ClientX, int ClientY ) ;										// 裏画面の内容を指定のウイン���ウに転送する
...
extern	int			SetUseDirectDrawFlag(							int Flag ) ;									// ( 同��果のSetUseSoftwareRenderModeFlag を使用して下さい )DirectDrawを使用するかどうかを設定する
...
extern	int			UpdateLayerdWindowForBaseImageRect(                   const BASEIMAGE *BaseImage, int x1, int y1, int x2, int y2 ) ;																		// 基本イメー��データの指定の範囲を使用して UpdateLayerdWindow を行う
...
extern	int			LoadSoundMemByResourceWithStrLen(    const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int BufferNum = 1 ) ;				// サウ���ドリソースからサウンドハンドルを作成する

// 設定関係関数
extern	int			SetUseSoftwareMixingSoundFlag(       int Flag ) ;																																	// サウンドの処理をソフトウエアで行うかどうかを設定する( TRUE:ソフトウエア  FALSE:ハードウエア( デフォルト ) )
extern	int			SetEnableXAudioFlag(                 int Flag ) ;																																	// サウンドの再生にXAudioを使用するかどうかを設定���る( TRUE:使用する  FALSE:使用しない( デフォルト ) )
