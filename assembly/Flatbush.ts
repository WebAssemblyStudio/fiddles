...
export default class Flatbush<T> {
    ArrayType: T;
    IndexArrayType: typeof Array;
    numItems: i64;
    nodeSize: i64;
    _levelBounds: f64[];
    minX: i64;
    ...
    constructor(numItems, nodeSize = 16, ArrayType = Array, data) {
        ...
        this.IndexArrayType = numNodes < 16384 ? Array<u16> : Array<u32>;
...
function axisDist(k, min, max): f64 {
    return k < min ? min - k : k <= max ? 0 : k - max;
}

// binary search for the first value in the array bigger than the given
function upperBound(value, arr): f64 {
    let i = 0;
    let j = arr.length - 1;
    while (i < j) {
        const m = (i + j) >> 1;
...
function sort(values, boxes, indices, left, right): void {
...
function swap(values, boxes, indices, i, j): void {
    const temp = values[i];
    values[i] = values[j];
    values[j] = temp;
...
function hilbert(x: f64, y: f64): f64 {
    let a = x ^ y;
    let b = 0xFFFF ^ a;
    let c = 0xFFFF ^ (x | y);
    let d = x & (y ^ 0xFFFF);
