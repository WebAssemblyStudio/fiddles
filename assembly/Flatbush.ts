...
export default class Flatbush {
    ...
    */
    ...
    constructor(numItems: u32, nodeSize: u16 = 16) {
        ...
        this.data = memory.allocate(8 + nodesByteSize + numNodes + Uint32Array.BYTES_PER_ELEMENT);

        // this.data = new ArrayBuffer(8 + nodesByteSize + numNodes * this.IndexArrayType.BYTES_PER_ELEMENT);
        
        // this._boxes = new this.ArrayType(this.data, 8, numNodes * 4);
        // this._indices = new this.IndexArrayType(this.data, 8 + nodesByteSize, numNodes);
        this._boxes = load<f64[]>(this.data + 8, numNodes * 4);
        this._indices = load<u32[]>(this.data + 8 + nodesByteSize, numNodes);
        this._pos = 0;
        this.minX = Infinity;
        this.minY = Infinity;
        this.maxX = -Infinity;
        ...
        store<u8>(this.data, 0xfb);
        store<u8>(this.data + 1, (VERSION << 4) + arrayTypeIndex);
        store<u16>(this.data + 2, nodeSize);
        store<u32>(this.data + 4, numItems);
    ...
    search(minX: f64, minY: f64, maxX: f64, maxY: f64, filterFn: (n: f64) => boolean) {
    ...
    neighbors(x: f64, y: f64, filterFn: (n: f64) => boolean, maxResults: f64 = Infinity, maxDistance: f64 = Infinity) {
        if (this._pos !== this._boxes.length) {
            throw new Error('Data not yet indexed - call index.finish().');
        }
