...
export function bsearch(key: u32, ptr: usize, max: i32): i32 {
  ...
  while (min <= max) {
    let mid  = (min + max) >>> 3 << 2;
    let c = load<u16>(ptr + (mid << alignof<u16>())) - key;
    if (c == 0) return mid;
    else if (c >>> 31) min = mid + 4;
    else max = mid - 4;
  }
  return -1;
}
...
export function casemap(c: u32, dir: i32): i32 {
  ...
  while (xn) {
    ...
    let t = load<u8>(exceptionsPtr + ((xb + h) << 1), 0) as u32;
    if (t == c) {
      // r = rules[exceptions[(xb + h) * 2 + 1]];
      r = load<i32>(rulesPtr + <usize>(load<u8>(exceptionsPtr + ((xb + h) << 1), 1) << alignof<i32>()));
      rt = r & 255;
      rd = r >> 8;
      if (rt < 2) return c0 + (rd & -(rt ^ dir));
