...
@inline
function rev32b(x: u32): u32 {
   let t: u32;
   t = x & 0x00FF00FF; x = rotl(t, 16) | (t ^ x);
   t = x & 0x0F0F0F0F; x = rotl(t,  8) | (t ^ x);
   t = x & 0x33333333; x = rotl(t,  4) | (t ^ x);
   t = x & 0x55555555; x = rotl(t,  2) | (t ^ x);
   return rotl(x, 1);
}
...
export function bench_rev32b(): u32 {
  let acc = 0;
  for (let i: u32 = 0; i < ITER; i++) {
    acc += rev32b(i);
  }
