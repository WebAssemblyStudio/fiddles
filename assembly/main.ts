...
export function mpt_verify(x: i32, y: i32): i32 {
  ...
  */
  ...
  keccak(keccakCtxBuf, address_data.dataStart, 20, hashOutputPtr);

  debug(hashOutputPtr);

  var address_hash = Uint8Array.wrap(hashOutput, 0, 32);
  ...
  **/
  ...
  for (let i = 0; i < 1; i++) {
    ...
    keccak(keccakCtxBuf, proof_decoded.children[i].dataStart, proof_decoded.children[i].length, hashOutputPtr);
    debug(hashOutputPtr);
    ...
    if (node_type == NodeType.Branch) {
      next_in_path = node_decoded[key_path[kp_i]];
      kp_i = kp_i + 1;

      // TODO: if key.length === 0 throw

      if (next_in_path.length == 2) {
        throw new Error('TODO: handle embedded node');
      }

      wantHash = next_in_path;
    }
