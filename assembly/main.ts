export function rotate(inputWidth: i32, inputHeight: i32, rotate: i32): void {
  ...
  let start: i32, d1: i32, d2Offset: i32;
  for (let d2 = d2Start; <i32>(d2 >= 0) & <i32>(d2 < d2Limit); d2 += d2Advance) {
    d2Offset = d2 * d2Multiplier;
    for (d1 = d1Start; <i32>(d1 >= 0) & <i32>(d1 < d1Limit); d1 += d1Advance) {
      store<u32>(offset + i, load<u32>(d1 * d1Multiplier + d2Offset));
      i += 4;
    }
  }
}
