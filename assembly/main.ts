...
let ts_info = new Array<i32>(12);

export function getInfo(): Array<i32> {    
    ts_info[0] = ts_rtpTimestamp;
    ts_info[1] = playMode;
    ts_info[2] = ts_FrameBufferLength;
    ts_info[3] = ts_packetSeq;
    ts_info[4] = ts_timestamp;
    ts_info[5] = ts_timestamp_usec;
    ts_info[6] = ts_timezone;
    ts_info[7] = ts_frameType;
    ts_info[8] = ts_SPSLength;
    ts_info[9] = ts_PPSLength;
    ts_info[10] = ts_framerate;
...
export function depacketize_H264(rtspInterleavedLength: u16, rtpHeaderLength: u16, rtpPayloadLength: u16): i32 {//, streamdata: StreamData): i32 {
    ...
    rtspInterleaved = new Uint8Array(rtspInterleavedLength);
    rtspInterleaved.forEach( function (a, i) {
        rtspInterleaved[i] = load<u8>(rtspInterleavedOffset + i);
    });

    rtpHeader = new Uint8Array(rtpHeaderLength);
    rtpHeader.forEach( function (a, i) {
        rtpHeader[i] = load<u8>(rtpHeaderOffset + i);
    });

    rtpPayload = new Uint8Array(rtpPayloadLength);
    rtpPayload.forEach( function (a, i) {
        rtpPayload[i] = load<u8>(rtpPayloadOffset + i);
    });    
