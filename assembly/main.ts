...
// TODO improve this
function ilogb(n: u64, base: i32): i32 {
  let t: u64 = 1;
  let e = 0;
  let b = u64(base);
  while (t <= n) {
...
export function utoa64_general(num: f64, radix: i32): string {
  ...
  if (isPowOf2) {
    do {
      offset--;
      store<u16>(res + (offset << 1), load<u16>(lut + (usize(x & mask) << 1)));
      x >>= shift;
    } while (x);
  } else {
    do {
      offset--;
      let q = x / base;
      store<u16>(res + (offset << 1), load<u16>(lut + (usize(x - q * base) << 1)));
      x = q;
    } while (x);
  }
  return changetype<string>(res); // retain
