...
export function utoa64hex_lut(num: u64): string {
	let x = u64(num);
...
export function utoa64hex_simple(num: u64): string {
  let x = u64(num);
	let offset = 63 - i32(clz(x)) >> 2;
  let res = __alloc(offset + 1 << 1, idof<string>());
	while (offset >= 0) {
...
export function bench_lut(): void {
  for (let i: u64 = 0; i < 10000000; i++) {
    utoa64hex_lut(i);
  }
}

export function bench_simple(): void {
  for (let i: u64 = 0; i < 10000000; i++) {
    utoa64hex_simple(i);
  }
}