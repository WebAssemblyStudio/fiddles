...
function rev64b(a: u64): u64 {
  a = rotr(a, 32);
  a = ((a >>> 16) & 0x0000ffff0000ffff) ^ ((a << 16) & ~0x0000ffff0000ffff);
  a = ((a >>>  8) & 0x00ff00ff00ff00ff) ^ ((a <<  8) & ~0x00ff00ff00ff00ff);
  a = ((a >>>  4) & 0x0f0f0f0f0f0f0f0f) ^ ((a <<  4) & ~0x0f0f0f0f0f0f0f0f);
  a = ((a >>>  2) & 0x3333333333333333) ^ ((a <<  2) & ~0x3333333333333333);
  a = ((a >>>  1) & 0x5555555555555555) ^ ((a <<  1) & ~0x5555555555555555);
  return a;
}
...
export function bench_rev64b(): u32 {
  let acc: u32 = 0;
  for (let i: u32 = 0; i < ITER; i++) {
    acc += <u32>rev64b(<u64>i);
  }
  return acc;
}
