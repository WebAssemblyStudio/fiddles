...
@inline function rempio2f(x: f32): i32 {
  ...
  return <i32>(s ? -_segment_hi : _segment_hi);
  */
}
...
@inline function __sindf(x: f64): f32 {
  ...
  */
  const c0 = 9.9999999692634277740e-1;
  const c1 = -1.6666650699202422675e-1;
  const c2 = 8.3320368751598070196e-3;
  const c3 = -1.9504022000675618505e-4;

  var xx = x * x;

  return <f32>(x * (c0 + c1 * xx + (c2 + c3 * xx) * (xx * xx)));
}
...
@inline function __cosdf(x: f64): f32 {
  ...
  */
  const c0 = 9.9999997242332292107e-1;
  const c1 = -4.9999856695848847717e-1;
  const c2 = 4.1655026884251524438e-2;
  const c3 = -1.3585908510113298585e-3;

  x *= x;
  return <f32>(c0 + c1 * x + (c2 + c3 * x) * (x * x));
}
...
logSin(<f32>(-i32.MAX_VALUE * Math.PI * 10000), sinf(<f32>(-i32.MAX_VALUE * Math.PI * 10000)));
logSin(<f32>(421657440.1234566), sinf(<f32>(421657440.1234566)));
logSin(<f32>(421657440 + 100000.123434355), sinf(<f32>(421657440 + 100000.123434355)));
logSin(<f32>(421657440 + 1000000), sinf(<f32>(421657440 + 1000000)));
logSin(<f32>(421657440 + 10000000), sinf(<f32>(421657440 + 10000000)));
logSin(-<f32>(421657440 + 10000000), sinf(-<f32>(421657440 + 10000000)));
logSin(-<f32>(421657440 + 100000002.123456789), sinf(-<f32>(421657440 + 100000002.123456789)));

export function bench(): f64 {
  var acc = 0.0;
  for (let i = 421657440; i < 421657440 + 1000000; i++) {
    acc += sinf(<f32>i);
  }
  return acc;
}