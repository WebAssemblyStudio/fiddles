...
export function mpt_verify(x: i32, y: i32): i32 {
  ...
  */
  ...
  let node_decoded = decode(proof_decoded.children[2].buffer);
  sayHello(55555);
  ...
  for (let i = 0; i < proof_decoded.children.length; i++) {
    // 
    // proof_decoded.children[i]

    // TODO: figure out what kind of trie node (branch, extension, or leaf) it is

    // RLP decode the trie node
    let node_decoded = decode(proof_decoded.children[i])
    sayHello(55555);
    sayHello(node_decoded.children.length);
    let node_type = getNodeType(node_decoded.children);
...
function getNodeType(node_data: RLPData[]): NodeType {
  ...
  if (node_data.length == 17) {
    ...
    //let key = stringToNibbles(node[0]);
    // only need the first nibble, not all of them
