declare function console_log(val: i32): void;
declare function console_i32(val: i32): void;
...
function casemap(c: u32, dir: i32): i32 {
  ...
  const mtPtr = mt.dataStart;
  const tabPtr = tab.dataStart;
  const ruleBasesPtr = ruleBases.dataStart;
  const rulesPtr = rules.dataStart;
  ...
  const exceptionsPtr = exceptions.dataStart;
...
export function toUpper(c: u32): i32 {
  return casemap(c, 1);
}

// console_log(casemap(1055, 0)); // П -> п
// console_log(casemap("A".charCodeAt(0), 0)); // A -> a