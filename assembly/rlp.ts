...
export function _decode(input: Uint8Array): Decoded {
    ...
    if (firstByte <= 0x7f) {
      ...
      sayHello(97);
        ...
        let length_view = new DataView(input.buffer, 1, llength);
        sayHello(77);
        let length = length_view.getUint64(0);
        sayHello(i32(length));
        //if (llength > 2) {
        ...
        //sayHello(55);
        //sayHello(length);
        let data = input.subarray(llength, i32(length) + llength);
        //sayHello(56);
        ...
        return new Decoded(new RLPData(data, null), input.subarray(i32(length) + llength));
